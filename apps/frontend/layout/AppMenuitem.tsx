...
const AppMenuitem = (props: AppMenuItemProps) => {
    ...
    // Helper function to render icon (PrimeIcons or FontAwesome)
    const renderIcon = (icon: string | IconDefinition | undefined, className?: string) => {
        if (!icon) return null

        // If it's an object (FontAwesome IconDefinition)
        if (typeof icon === "object" && "iconName" in icon) {
            return <FontAwesomeIcon icon={icon as IconDefinition} className={className} />
        }

        // If it's a string (PrimeIcons)
        ...
        <li className={classNames({ "layout-root-menuitem": props.root, "active-menuitem": active })}>
            ...
            {(!item!.to || item!.items) && item!.visible !== false ? (
                <a href={item!.url} onClick={(e) => itemClick(e)} className={classNames(item!.class, "p-ripple")} target={item!.target} tabIndex={0}>
                    {renderIcon(item!.icon, "layout-menuitem-icon")}
